...
pub fn print_banner(
    ...
    if let Some((upnp_host, upnp_port)) = upnp_endpoint {
        println!(
            "  {electric_cyan}upnp    {reset} {dim}→{reset} {electric_purple}http://{upnp_host}:{upnp_port}{reset}"
        );
    }
    println!(
